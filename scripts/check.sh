...
bash -n install.sh uninstall.sh scripts/build-app.sh scripts/check.sh scripts/security-audit.sh
plutil -lint swift/Info.plist entitlements.plist
plutil -convert xml1 -o /dev/null docs/privacy/network-calls.json
...
grep -q 'fazzilka/voice_to_text' install.sh
grep -q '^REF="${VOICE_TO_TEXT_REF:-\$SOURCE_REF}"$' install.sh
grep -q '^EXPECTED_SOURCE_COMMIT="${VOICE_TO_TEXT_SOURCE_COMMIT:-}"$' install.sh
...
grep -q 'maximumRecordingSeconds: TimeInterval = 40 \* 60' core/Sources/VoiceToTextCore/RecordingPolicy.swift
grep -q 'VoiceToTextCore' swift/Package.swift

./scripts/security-audit.sh
git diff --check
printf 'VoiceToText checks passed (v%s).\n' "$app_version"

...
for command_name in git grep file sed perl; do
...
while read -r mode _ _ tracked_file; do
    ...
    head -n 1 "$tracked_file" | grep -Eq '^#!/bin/(ba)?sh$' \
...
if grep -Eni \
    ...
    install.sh uninstall.sh scripts/build-app.sh scripts/check.sh scripts/install-local.sh; then
...
if perl -CSD -ne '
    ...
    END { exit($found ? 0 : 1) }
' $(git ls-files 'install.sh' 'uninstall.sh' 'scripts/*' 'core/*' 'swift/Sources/*' '.github/*'); then

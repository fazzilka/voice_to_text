...
VoiceToText is designed for local dictation.
...
- Successful transcripts, timing statistics, corrections, and preferences are
  stored under `~/Library/Application Support/VoiceToText`.
...
VoiceToText uses the network to download the speech model through
...
The machine-readable inventory of network calls is maintained in
[`docs/privacy/network-calls.json`](docs/privacy/network-calls.json).
...
macOS Keychain (service `com.fazzilka.voicetotext.ai`), never in preferences

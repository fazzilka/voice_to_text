...
Тест `paste` отдельно проверяет три стиля пунктуации на русском и английском:
объединение паузных точек, сохранение `?`, `!`, многоточий, десятичных чисел и
сокращений, а также голосовые команды и сохранение выбранного режима.
...
9. в непрерывном режиме произнести несколько фраз с паузами и проверить, что
   внутренние точки стали запятыми, а финальная точка сохранилась;
10. проверить вопрос, восклицание, многоточие, число `1.2` и сокращение `см.`;
11. включить режим голосовых команд и проверить точку, запятую и новый абзац;
12. проверить основной и альтернативный hotkey;
13. проверить историю и повторную вставку;
14. сменить input device;
15. выполнить sleep/wake и повторить диктовку;
16. выключить сеть и убедиться, что локальная диктовка продолжает работать;
17. включить AI cleanup только с тестовым текстом и проверить raw fallback;
18. перезапустить агент из control panel.

...
- три стиля пунктуации: стандартный, непрерывный и управляемый голосовыми
  командами;
...
- **Стандартный** сохраняет пунктуацию, которую вернула модель.
- **Непрерывный** (режим beta по умолчанию) превращает одиночные точки между
  фразами в запятые, но сохраняет вопросы, восклицания, многоточия, числа и
  распространённые сокращения. Финальная точка остаётся.
- **По голосовым командам** дополнительно понимает команды `точка`, `запятая`,
  `вопросительный знак`, `восклицательный знак`, `новое предложение`, `новый
  абзац` и их английские аналоги.

Обработка выполняется локально после распознавания. Она применяется повторно
после необязательной AI-чистки, поэтому выбранный стиль остаётся итоговым.
...
flowchart LR
    ...
    C --> U["выбранный стиль пунктуации"]
    U --> O["необязательная AI-чистка"]
    O --> U2["финальная нормализация пунктуации"]
    U2 --> D["вставка в исходное приложение"]
